...
(custom-set-variables
 ...
 '(package-selected-packages
   '(cape consult consult-dir doric-themes ef-themes embark embark-consult emms evil evil-collection evil-numbers gnuplot helm hydra ledger-mode magit marginalia orderless ox-pandoc tempel vertico which-key))
...
(defun my-theme-dark (theme &optional req)
  ...
  (cond
   ...
   ((or (eq theme 'doric-dark)
	(eq theme 'doric-obsidian))
    (load-theme 'my-override-dark3 t))
   (t
...
(when (and (file-exists-p "~/mp3")
  ...
  (define-key emms-playlist-mode-map (kbd "C-c C-n") 'emms-next)
  (define-key emms-playlist-mode-map (kbd "C-c C-p") 'emms-previous)
